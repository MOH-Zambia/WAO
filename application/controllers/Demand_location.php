...
class Demand_location extends Generic_home {
    ...
    public function collectionMapping() {
        $this->breadcrumbs->push('Demand Locations', '/demand_location/listAll');
        $data['demand_location_list'] = $this->demand_location_model->get_list();
        $data['demand_location_id'] = $this->demand_location_model->get_demand_location_dropdown();
        $this->load->view('demand_location_collection_view', $data);
        $this->load->view('footer');
    }
    ...
    public function postMapping() {
        $this->breadcrumbs->push('Demand Locations', '/demand_location/listAll');
        $data['demand_location_list'] = $this->demand_location_model->get_list();
        $master_demand_location_id = $this->input->get_post('demand_location_id');

        foreach ($this->input->get_post('demandLocations') as $item) {
            $this->worker_demand_model->update_location($item, $master_demand_location_id);
            if ($master_demand_location_id != $item) {
                $this->demand_location_model->delete($item);
            }
        }
        $this->load->view('demand_location_list_view', $data);
        $this->load->view('footer');
